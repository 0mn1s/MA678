...
```{r}
set.seed(100)
lr_1 <- stan_glm(vote ~ x, data=hibbs,refresh=0) |> print()
lr_2 <- lm(vote ~ x, data=hibbs) |> print()
...
plot_10 <- ggplot(sim_1,aes(x,y))+
  ...
  geom_point(data=sim_1 %>% filter(z==1),
             pch=1,
             size=5) +
  geom_abline(mapping = NULL, b_hat[1] + b_hat[2], b_hat[3],col="red")
  theme_classic()
plot_10 
...
```{r}
fit_105 <- stan_glm(kid_score ~ mom_age, data = kidiq, refresh = 0)
plot(kidiq$mom_age,kidiq$kid_score)
abline(coef(fit_105), col = "red") 
print(fit_105)
...
# However, this is so wrong based on our daily experieces, since a 45 years old
# pregnant woman is unlikey to have a smart baby. It mwans that there is something 
